...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (target) {
    target.open = request.storageInfo.isOpen;
  }
});

async function initState() {
  for (const item of tools.value) {
    const method = `get${item.key[0].toUpperCase() + item.key.slice(1)}Info`;
    const res = await storage[method as keyof IObjectKeys]();
    console.log(res);
    if (res) {
      item.open = res.isOpen;
    }
  }
}
...
<template>
  <div class="container px-2 py-2 bg-orange-50 overflow-y-scroll box-border">
    <header class="box-border text-lg font-bold flex justify-center items-center py-2">Notion Faster</header>
    <main>
